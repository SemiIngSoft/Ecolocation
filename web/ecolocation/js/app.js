...
$(function() {
    ...
    $("#btnDescargar").on('click', function(event) {
      ...
      if (pedidotext.trim() != '') {
        ...
        $.ajax({
          ...
          .done(function(data) {
            console.log(data);
            window.location = "pedidohistorico.php?done=1";
  ...
  if($(".pedidohistorico").length){
    $("#example").DataTable();
    $(".close").on('click',  function(event) {
      event.preventDefault();

    });
    function mapCreate(lat, lng, json){
      var map = null;
      var marcador;
      var myLatlng;
      ...
      marcador.setMap(map);
      console.log(json);
      console.log(json[0].id_peticion);
      $(".idPedido").html(`<strong># Pedido</strong><p>${json[0].id_peticion}</p>`);
      $(".fecha").html(`<strong>Fecha: </strong><p>${json[0].fecha_peticion}</p>`);
      $(".domicilio").html(`<strong>Domicilio: </strong><p>${json[0].domicilio_ciuddano}</p>`) ;
    }
    function test (){
      console.log("Hola");
    }
    //$('.open-modal').click(function(e) {
      /* Act on the event */
    //  e.preventDefault();
    //  $("#modal-infoC").modal("show");
        //$("#info").html(`<h1> Hoala </h1>`);
    //});
    $('#modal-infoC').on('show.bs.modal', function () {
      console.log("Hpooalaso");
      //map()
    });
    $(".table tbody").on('click','.btn', function(event) {
      event.preventDefault();
      var currow = $(this).closest('tr');
      var idPedido = currow.find('td:eq(0)').text();
      $.ajax({
        url: '../php/functions/get_pedido.php',
        type: 'POST',
      ...
      .done(function(data) {
        ...
        $("#modal-infoC").modal("show");
        var ubicacion = json[0].ubicacion_ciudadano.split(",")
        mapCreate(ubicacion[0], ubicacion[1], json);
        //$("#info").html(`<h1> Haola </h1>`);
        ...
        console.log(json[0].ubicacion_ciudadano);
      })
      .fail(function() {
        console.log("error");
      })
